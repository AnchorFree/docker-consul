...
bucket_list=`/opt/s3cmd/s3cmd ls s3://$bucket/ 2>/dev/null`
s3_avail=$?
...
/opt/s3cmd/s3cmd get s3://$bucket/peers*.json /consul/config/

# Consul first run
if [ -f /consul/firstrun ]; then
    echo "{ \"datacenter\": \"$dc\" }"  > /consul/config/dc.json
    echo "{ \"start_join\": [\"$ip\"] }" > /consul/config/$peer.json
    /opt/s3cmd/s3cmd get s3://$bucket/main.json /consul/config/main.json
	...
	/opt/s3cmd/s3cmd get s3://$bucket/key.json /consul/config/key.json
    fi
